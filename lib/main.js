...
const serveDebugger = (path = '/debug', isDev) => {
    router.use(path, express.static(isDev ? './dashboard/build' : './node_modules/express-logger/dashboard/build'))
    return router;
};

const serveFSLogger = (path = '/logs') => {
    const filePath = env === 'dev' ? nodeModules() + '/../data' : nodeModules() + '/express-logger/data';
    router.use(`${path}/:file`, (req, res, next) => {
        const file = req.params.file;
        if (!file) {
            res.status(400).send('File not found');
        }
