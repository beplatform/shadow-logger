...
// const regexEscape = (sltr: string): string => {
//     return sltr.replace(new RegExp('[.\\\\+*?\\[\\^\\]$(){}=!<>|:\\-]', 'g'), '\\$&');
// };
...
const serveS3Logger = async (logsApiUrl, bucket, path) => {
    const router = express.Router();
    ...
    router.use(`${logsApiUrl}/:file`, async (req, res) => {
        ...
        try {
            const data = await s3Client.send(searchParams);
            let obj = JSON.parse(await streamToString(data.Body));
            if (file === 'index.json') {
                obj = parseIndex(req, res, obj);
            }
            res.json(obj);
