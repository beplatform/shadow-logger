...
const serveS3Logger = (logsApiUrl, bucket, s3Prefix, awsClientConfig) => {
    let s3Client;
    if (awsClientConfig) {
...
const shadowDebugger = (options) => {
    ...
    let { debugApiUrl, debuggerBuildPath, s3Bucket, s3Prefix, awsClientConfig } = options;
    if (!debugApiUrl) {
        debugApiUrl = '/logs'
    }
    ...
    const router = express.Router();
    router.use(express.static(debuggerBuildPath));
    router.use(serveS3Logger(debugApiUrl, s3Bucket, s3Prefix, awsClientConfig));
