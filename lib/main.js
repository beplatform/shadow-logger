...
const configMiddleWare = (config = {writerType: 'fs', path: './logs', ingore: []}) => {
    ...
    const writeToS3 = (log) => {
        ...
        const uploadIndex = (index, log) => {
            ...
            s3Client.upload(uploadIndexParams, function (err, data) {
                if (err) {
                    console.log(err);
        ...
        const uploadLog = (log) => {
            ...
            s3Client.upload(uploadLogParams, function (err, data) {
                if (err) {
                    console.log(err);
                    console.log('Error uploading log: ', data);
                } else {
        ...
        s3Client.getObject(searchParams, function(err, data) {
            if (err) {
                ...
                const indexArray = JSON.parse(buf.toString());
