...
const configMiddleWare = (config = {writerType: 'fs', path: './logs', ingore: []}) => {
    ...
    if (writerType === 's3') {
        ...
        if (!s3.key) {
            console.error('Need s3 key configuration');
        }
        if (!s3.region) {
            s3.region = 'us-east-1';
        }
    ...
    const writeToAWS = (log) => {
        ...
        AWS.config.update({region: s3.region});
        const s3Service = new AWS.S3();
        let uploadParams = {Bucket: s3.bucket, Key: s3.key};
        s3Service.getObject(uploadParams, function(err, data) {
